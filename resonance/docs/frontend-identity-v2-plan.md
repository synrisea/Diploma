...
- Buttons: primary actions use `bg-brand-500 text-brand-ink hover:bg-brand-600`, full pill shape (`rounded-full`) for calls to action, `rounded-xl` for form inputs. On a page with several actions (e.g. Settings), keep it to exactly two button languages — a filled pill for the one primary action per section, plain color-coded text links for everything else (brand color for a neutral secondary action like "Change email", `sentiment-negative` for a destructive one like "Remove"/"Revoke") — not bordered/outlined buttons as a third style.
