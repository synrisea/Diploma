...
const CONNECTIONS_API_BASE_URL = config.connectionsApiBaseUrl;
...
export async function registerDevice(accessToken: string, token: string, platform: string): Promise<void> {
  await authedFetch('/api/connections/devices', accessToken, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
...
export async function unregisterDevice(accessToken: string, token: string): Promise<void> {
  await authedFetch(`/api/connections/devices/${encodeURIComponent(token)}`, accessToken, { method: 'DELETE' });
}
