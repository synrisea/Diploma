...
export function CommentList({ placeId }: { placeId: string }) {
  ...
  if (isError) return <p className="text-sm text-rose-700">Couldn't load comments.</p>;
  if (!data || data.length === 0) return <p className="text-sm text-stone-400">No comments yet — be the first to leave one.</p>;

  return (
    <ul className="flex flex-col gap-3">
      {data.map((comment) => (
        <li key={comment.id} className="rounded border border-stone-200 p-3 text-sm text-stone-700">
          <p>{comment.comment}</p>
          <p className="mt-1 text-xs text-stone-400">{new Date(comment.createdAt).toLocaleDateString()}</p>
        </li>
      ))}
    </ul>
  );
